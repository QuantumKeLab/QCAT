...
if __name__ == '__main__':
    ...
    If = np.random.normal(f[0], std_dev, num_samples)    
    Qf = np.random.normal(f[1], std_dev, num_samples) 
    # ( ["state","mixer","index"], 
    data =  np.array([[Ig, Qg], [Ie, Qe], [If, Qf]])
    coords= [ ("prepared_state", [0,1,2]), ("mixer",["I","Q"]), ("index",np.arange( num_samples ))]
    ...
    plt.plot( dataarray.sel(mixer="I", prepared_state=2).values, dataarray.sel(mixer="Q", prepared_state=2).values, "o" )
